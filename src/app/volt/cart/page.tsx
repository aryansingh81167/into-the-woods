...
import { Trash2, Plus, Minus, MapPin, HardDrive, Award } from 'lucide-react';
...
export default function VoltCartPage() {
  const { cartItems, updateQuantity, removeFromCart, getCartTotal, clearCart } = useVoltCart();
    ...
    <div className="bg-background min-h-screen">
      <div className="container mx-auto max-w-lg px-4 py-8">
        <div className="flex items-center gap-3 mb-6">
          <Link href="/volt" className="text-primary">
             <HardDrive className="h-7 w-7" />
          </Link>
          <h1 className="text-3xl font-bold font-headline text-primary">Your Volt Cart</h1>
        </div>

        {cartItems.length === 0 ? (
          <Card>
            <CardContent className="py-20 text-center">
              <p className="text-lg text-muted-foreground mb-4">Your Volt cart is empty.</p>
              <Button asChild>
                <Link href="/volt">Shop Electronics</Link>
              </Button>
            </CardContent>
          </Card>
        ) : (
